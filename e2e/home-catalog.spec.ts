...
import { featuredCliInstallCommand, homeHeading } from './fixtures/home-copy'
...
test.describe('Home catalog', () => {
  test('loads mocked registry packages', async ({ page }) => {
    ...
    await expect(page.getByTestId('home-cli-install-terminal')).toContainText(featuredCliInstallCommand)
  ...
  test('scrolls to the CLI quickstart from a hash URL', async ({ page }) => {
