---
name: maiconfz--plan-refiner--plan-refinement
description: >-
...
- `draft-plan` (string): Draft plan as markdown text or a workspace path to a markdown file.
- `refine-mode` (string): interactive or automatic; ask which to use when missing.
- `goal` (string): Optional stated goal or acceptance text to check the plan against.
- `user-clarifications` (string): Optional user answers accumulated across clarification loops.

### Outputs

- `refined-plan` (string): Revised markdown plan after the selected refiner finishes.
- `open-questions` (string): Remaining questions at handoff; blocking items first when present.
- `assumption-log` (string): Assumptions from automatic mode; empty after interactive refine.
