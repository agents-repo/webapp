---
name: maiconfz--context-token-reduction--token-reduction-advisor
description: >-
  Ask-first drafts of an ordered context-token reduction plan from a footprint
  report. Plan-only; does not edit files. Use when the user needs the
  maiconfz--context-token-reduction--token-reduction-advisor workflow.
---
# Overview
...
- `footprint-report` (string): Markdown footprint report from token-footprint-analyst.
- `planning-consent` (boolean): True only when the user consented to reduction planning.
- `user-clarifications` (string): Optional user answers from prior clarification loops.

### Outputs

- `reduction-plan` (string): Markdown reduction plan; empty when consent is missing.
