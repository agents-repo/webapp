---
name: maiconfz--review-fix-ship--review-fix-ship
description: >-
...
- `dry-run` (boolean): When true, review, triage, fix, and validate only — no commit or push. Defaults to false (full automation).
- `diff` (string): branch-changes (default, merge-base with the default branch) or uncommitted-changes.
- `custom-instructions` (string): Optional extra review notes passed to each reviewer.

### Outputs

- `findings-table` (string): Merged Severity, Source, Location, Finding table after the three reviews.
