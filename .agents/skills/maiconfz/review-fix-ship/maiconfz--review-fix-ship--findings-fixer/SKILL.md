---
name: maiconfz--review-fix-ship--findings-fixer
description: >-
  Triages merged review findings, applies minimal fixes, runs project checks,
  then commits and pushes the feature branch unless dry-run is true.
---
# Overview
...
- `findings-table` (string): Merged review findings table from the calling flow.
- `dry-run` (boolean): When true, triage, fix, and validate only — no commit or push. Defaults to false (full automation).
- `diff` (string): branch-changes (default, merge-base with the default branch) or uncommitted-changes.
- `custom-instructions` (string): Optional extra notes from the caller.

### Outputs

- `triage-table` (string): Markdown table with source, location, outcome, and rationale per finding.
- `handoff-summary` (string): Branch, commit SHA when pushed, what shipped or why ship was skipped.
