...
description: >-
  Inventory host-tree context-token waste, ask consent, then draft an
  ordered reduction plan. Planning only; does not implement.
version: 1.0.0
license: MIT
agents:
...
inputs:
  ...
  - name: user-clarifications
    type: string
    description: >-
      Optional user answers accumulated across clarification loops.
outputs:
