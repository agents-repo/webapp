...
description: >-
  Runs code, bug, and security reviews (Cursor skills when available),
  merges findings, then fixes, commits, and pushes. Complements GitHub PR
  thread triage.
version: 1.0.0
license: MIT
agents:
...
inputs:
  - name: dry-run
    type: boolean
    description: >-
      When true, review, triage, fix, and validate only — no commit or
      push. Defaults to false (full automation).
  - name: diff
    type: string
    description: >-
...
outputs:
  - name: findings-table
    type: string
    description: >-
      Merged Severity, Source, Location, Finding table after the three
      reviews.
