...
- Package card footer actions (CLI, Use in chat, Download, View) include short
  visible labels. Below Bootstrap `md`, and on `md+` viewports whose primary
  pointer cannot hover, the labels stay visible and the footer may wrap.
  On hover-capable `md+` the footer stays on one row (`flex-wrap: nowrap`)
  and labels stay collapsed at rest, then expand on that control's hover,
  keyboard focus (`:focus-visible`), or while `aria-expanded` is true. jsdom
  tests assert the label text and footer class in the DOM, not the collapse
  media query.
