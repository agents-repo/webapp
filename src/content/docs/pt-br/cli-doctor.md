...
- Antes de depurar `install` com falha ou falhas de `agents:verify` no PR baseline (use `agents-repo doctor --skip-artifact-download` para checagens no estilo verify); também útil antes de `agents:ci` local ao validar locks ou extracts
