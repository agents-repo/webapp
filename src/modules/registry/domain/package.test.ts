...
describe('toPackageSlug', () => {
  ...
  it('trims whitespace and sanitizes awkward characters for DOM ids', () => {
    expect(toPackageSlug(' agents-repo ', 'hello/agent')).toBe('agents-repo--hello-agent')
    expect(toPackageSlug('Foo Bar', 'pkg.id')).toBe('foo-bar--pkg-id')
  })

  it('falls back to unknown when a segment is empty after sanitization', () => {
