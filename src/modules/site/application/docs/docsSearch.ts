...
interface DocSearchIndexEntry {
  readonly entry: DocManifestEntry
...
function matchRank(
  indexEntry: DocSearchIndexEntry,
  queryLower: string,
...
export function searchDocPages(query: string, options?: DocSearchOptions): DocSearchResult[] {
  ...
  return matches.slice(0, maxResults).map(({ indexEntry, rank }) => {
    ...
    return {
      slug: entry.slug,
      title: entry.title,
      href: getDocDetailPath(entry.slug),
      snippet,
    }
  })
}

export { stripDocMarkdownForSearch }
